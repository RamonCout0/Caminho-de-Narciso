...
func check_collisions():
	# Verifica corpos (CharacterBody2D do player)
	for body in detector.get_overlapping_bodies():
		if body.is_in_group("player"):
			body.call("morrer_instantaneamente", "Pisou no Bispo!")
			return

# Handler unificado — conectado no .tscn ao CollisionDetector
func _on_collision_detector_body_entered(body: Node2D) -> void:
	if body.is_in_group("player"):
		body.morrer_instantaneamente("Pisou no Bispo!")

func _on_collision_detector_area_entered(area: Area2D) -> void:
	var owner_node = area if area.is_in_group("player") else area.get_parent()
	if owner_node.is_in_group("player"):
		owner_node.morrer_instantaneamente("Pisou no Bispo!")
