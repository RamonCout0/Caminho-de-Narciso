...
func _on_area_entered(area: Area2D): if area.is_in_group("player"): GameManager.take_damage(GameManager.max_hp)
func check_collisions():
	for area in detector.get_overlapping_areas(): if area.is_in_group("player"): GameManager.take_damage(GameManager.max_hp)
